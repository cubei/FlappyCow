...
public class GameOverScreen extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences medaille_save = this.getSharedPreferences(
                MainActivity.medaille_save, 0);
        int medaille = medaille_save.getInt(MainActivity.medaille_key, 0);

        SharedPreferences.Editor editor = medaille_save.edit();

        if (points >= gold) {
            ((ImageView) findViewById(R.id.medaille)).setImageBitmap(Sprite
                    .createBitmap(getResources().getDrawable(R.drawable.gold),
                            this));
            if (medaille < 3) {
            ...
            ((ImageView) findViewById(R.id.medaille))
                    .setImageBitmap(Sprite.createBitmap(getResources()
                            .getDrawable(R.drawable.silver), this));
            if (medaille < 2) {
            ...
            ((ImageView) findViewById(R.id.medaille))
                    .setImageBitmap(Sprite.createBitmap(getResources()
                            .getDrawable(R.drawable.bronce), this));
            if (medaille < 1) {
